...
- **Flash Attention** — Uses `flash_attn_with_kvcache` (FA3, SM90) or `flash_attn.cute` (FA4, SM90+/SM100). Both paths support causal sliding-window attention; FA4 also supports per-head learnable attention sinks used by GPT-OSS. Requires `is_paged_attn=True` in the KV cache context. Forces `num_splits=1` for determinism.
